...
/*   Updated: 2023/08/05 18:25:49 by jsuarez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_matcher(char *str, char *set)
{
	...
	while (*str++ != 0 && flg == 0)
	{
		while (*conv != 0 && flg == 0)
		{
			...
			{
				flg = 1;
				printf("D:\t the caracter matched was %c\n", *str);
			}
		}
		conv = set;
	}
...
static int	ft_setter(char *fstr, char **off, char *off_pc, t_fstr *root)
{
	t_nd	nd;
	t_lm	lm;
	/*ft_setter is a too long function, we must reduce it.*/
	if (*off == off_pc)
	{
		nd.is_lf = 0;
		lm.off = *off;
...
int	ft_parser(char *fstr, t_fstr *root)
{
	...
	if (fstr != off)
	{
		printf("D:\tWe are here with this parameters\t%s\t%d\t%s\n", fstr,
			root->q_nd, off);
		ft_set_nd(&fstr, root, &off);
	}
	return (0);
